...
export const getAccountUsage = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Comments are now stored in the 'comments' table, not in messages
    ...
    // Count comments where postId matches any of the user's visible post IDs
    const totalComments = allComments.filter(comment => 
      postIdSet.has(comment.postId.toString())
    ).length;
